...
const Showtime = () => {
  ...
  const filteredShowtimes = showtime
    .map((movie) => {
      const validShowTimeList = (movie.showTimeList || []).filter((show) => {
        if (show.showDate !== selectedDate) return false;

        const showDateTime = new Date(`${show.showDate}T${show.startTime}`);
        const showStartMinus15 = new Date(showDateTime.getTime() - 15 * 60000);

        return now < showStartMinus15;
      });

      return validShowTimeList.length > 0
        ? { ...movie, showTimeList: validShowTimeList }
        : null;
    })
    .filter(Boolean); // loại bỏ phim không còn suất chiếu hợp lệ
