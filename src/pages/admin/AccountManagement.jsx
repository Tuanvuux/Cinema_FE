...
const AccountManagement = () => {
  ...
  const handleToggleDeleteStatus = async (account) => {
    ...
    try {
      ...
      setSelectedAccountForAction(null);
      addToast(
        isCurrentlyActive ? "Đã khóa tài khoản!" : "Đã khôi phục tài khoản!",
        "success"
      );
    } catch (error) {
      addToast(
  ...
  const confirmBulkDelete = async () => {
    ...
    try {
      ...
      addToast(
        `Đã khóa ${selectedAccount.length} tài khoản thành công!`,
        "success"
      );
    } catch (error) {
  ...
  const confirmBulkRestore = () => {
    ...
    Promise.all(updatePromises)
      .then(() => {
        ...
        setSelectedAccount([]);
        addToast(
          `Đã khôi phục ${selectedAccount.length} tài khoản thành công!`,
          "success"
        );
      })
      .catch((error) => {
              ...
              <table className="min-w-full bg-white border border-gray-200 rounded-lg overflow-hidden">
                <thead>
                  <tr className="bg-gray-100 border-b">
                    ...
                    <th className="p-3 text-left text-sm font-medium text-gray-600 uppercase tracking-wider hidden sm:table-cell">
                      Họ tên
                    </th>
                    <th className="p-3 text-left text-sm font-medium text-gray-600 uppercase tracking-wider hidden sm:table-cell">
                      Email
                ...
                <tbody className="divide-y divide-gray-200">
                  {currentAccount.map((account) => (
                    ...
                    >
                      ...
                      <td className="pp-3 font-medium text-center text-gray-900">
                        {account.userId}
                      </td>
                      <td
                      ...
                      >
                        <div className="flex items-center">
                          ...
                          <span
                            className={!account.isActive ? "line-through" : ""}
                          >
                            {account.username}
                          </span>
                          {!account.isActive && (
                      ...
                      <td
                        className={`p-3 text-left hidden sm:table-cell ${
                          !account.isActive ? "text-gray-400" : "text-gray-600"
                        }`}
                      >
                        {account.fullName}
                      </td>
                ...
                <div className="p-6">
                  ...
                  <form className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
                    ...
                    <div>
                      ...
                      <input
                        value={selectedAccountForAction.role || ""}
                        disabled
                        className="w-full border border-gray-300 rounded-lg py-2.5 px-3.5 focus:outline-none focus:ring-2 focus:ring-gray-600 focus:border-gray-600 shadow-sm transition-all duration-200 bg-gray-100"
                      />
                    </div>
